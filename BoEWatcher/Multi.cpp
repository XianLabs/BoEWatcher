...
BOOL EnableMulti() //currently *not* working for x64 processes, or PoE atleast. May need to destroy it from the creating thread due to security descriptors
{
	...
	for (unsigned int i = 0; i < infoSet->count; i++) 
	{
		...
		if (wcsstr(nameBuffer, L"PoERunMutexA") != NULL) 
		{
			HANDLE handle;
